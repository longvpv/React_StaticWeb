...
class MainContent extends PureComponent {
	...
	handleShowClick = (number) => {
        ...
        if ( onFilterShowOffChange ) {	onFilterShowOffChange(number)	}
	...
	handleSoftClick = (filterName) => {
		...
		if ( onFilterSoftingChange ) {	onFilterSoftingChange(filterName);	}
		
		if (filterName === 'salePrice') {
			const sortingName = 'Price'; this.setState({sortingName})
		} else if (filterName === 'name') {
			const sortingName = 'Product Name'; this.setState({sortingName})
		} else {const sortingName = 'Default Sorting'; this.setState({sortingName})}		
	}
	...
	handleNextPageClick = () => {
        ...
        if ( onNextPageClick ) {onNextPageClick()};
	}
	handleClose = () => {
...
<div className="product_sorting_container product_sorting_container_top">
	<ul className="product_sorting">
		<li>
			...
			<ul className="sorting_type">
				<li className="type_sorting_btn" onClick={() => this.handleSoftClick('original-order')}><span>Default Sorting</span></li>
				<li className="type_sorting_btn" onClick={() => this.handleSoftClick('salePrice')}><span>Price</span></li>
				<li className="type_sorting_btn" onClick={() => this.handleSoftClick('name')}><span>Product Name</span></li>
			</ul>
		</li>
		<li>
			<span>Show</span>
			...
			<ul className="sorting_num">
				<li className="num_sorting_btn" onClick={() => this.handleShowClick(6)}><span>6</span></li>
				<li className="num_sorting_btn" onClick={() => this.handleShowClick(12)}><span>12</span></li>
				<li className="num_sorting_btn" onClick={() => this.handleShowClick(24)}><span>24</span></li> 
			</ul>
		</li>
	</ul>
	<div className="pages d-flex flex-row align-items-center">
		...
		<div id="next_page" className="page_next" onClick={this.handleNextPageClick}><i className="fa fa-long-arrow-right" aria-hidden="true"></i></div>
	</div>

</div>
    

	<ul className="product-grid" data-isotope='{ "itemSelector": ".product-item", "layoutMode": "fitRows" }'>
                    {products.map(product => {
					
                        return (
                            <li key={product.id} className="product-grid__item-width" style={{width: '33.3333%'}}>
								<div className="product-item" >
                                   ...
                                   {product.salePrice !== product.originalPrice ? (
											<span className="sale_price">{product.originalPrice}</span>) : ''	}
	...
	<span className="showing_results">Showing 1–3 of {itemShowing} results</span>
	<div className="pages d-flex flex-row align-items-center">
		<div className="page_current">
				<span>{filters._page}</span>
		</div>
		<div className="page_total"><span>of</span> {totalPage}</div>
		<div id="next_page_1" className="page_next" onClick={this.handleNextPageClick}><i className="fa fa-long-arrow-right" aria-hidden="true"></i></div>
	</div>
